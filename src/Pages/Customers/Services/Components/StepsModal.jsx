...
const StepsModal = ({ currentStep = 1 }) => {
  ...
  return (
    <>
    <div className='flex gap-[1px] items-start justify-between px-10'>
        {STEPS.map((step, index) => (
            <React.Fragment key={step.id}>
                <StepCircle stepId={step.id} currentStep={currentStep} />
                {index < STEPS.length -1 && 
                <div className='flex-1 border-t-2 border-dashed border-neutral-300 mt-4 mx-2'/>
                }
            </React.Fragment>
        ))}
    </div>
...
//         <div className="px-10">
//           {currentStep === 1 && <div>{/* Service step content */}</div>}
//           {currentStep === 2 && <div>{/* Partner step content */}</div>}
//           {currentStep === 3 && <div>{/* Schedule step content */}</div>}
//         </div>
